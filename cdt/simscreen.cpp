...
#define CFG_SCREEN_NUM          ( 3 )                           //模拟屏数量
#define CFG_LIMIT_VALUE_YC2YX   ( 0.1 )                         //多个遥测对应一个遥信门槛值
...
TY_YX_MB_MAP_LINK yxMbMapLink_Simscreen[CFG_SCREEN_NUM];    //模拟屏  遥信表
TY_YC_MB_MAP_LINK ycMbMapLink_Simscreen[CFG_SCREEN_NUM];    //模拟屏  遥测表
TY_YC_MB_REMAP_LINK ycMbREMapLink_Simscreen[CFG_SCREEN_NUM];//模拟屏  遥测重定义表
...
enum VAL_STATUS MultiVal2OneYX(TY_YX_MB_MAP *pYxMbMap, ushort *pval); //多个值对应一个遥信量
...
void loadCfg(char* cfgfile)
{
    ...
    printf("loadDevice...\n");
    loadDevice(fp);
    for (i = 0; i < CFG_SCREEN_NUM; i++) {
        loadYxMap(fp, &yxMbMapLink_Simscreen[i]);	  //解析遥信量定义
        loadYcMap(fp, &ycMbMapLink_Simscreen[i]);	  //解析遥测量定义
        loadYcREMap(fp, &ycMbREMapLink_Simscreen[i]); //解析遥测量重定义表
        yxFlag_Simscreen |= (yxFlag << i);
        ycFlag_Simscreen |= (ycFlag << i);
    }
...
{
    DWORD lpThreadId = 0;
    HANDLE h0 = CreateThread(NULL, 0, modbusScanThread, NULL, 0, &lpThreadId);
...
void convertMbReg(uchar *sbuf, uint Serail_NUM)
{
    ushort *pbuf;
    float val;
    enum VAL_STATUS val_YXstatus = NOT_Multi;
    ...
    if (mbRegStart < CFG_YC_START_SEQ) {
        for (; pyxlink != NULL; pyxlink=pyxlink->next) {
            ...
            if (mbDevAddr==MachineAddr && (mbRegStart+mbRegSize-mbRegSurplus)==pYxMbMap->yxSeqStart) {
                ...
                if (pyxlink->next != NULL && pYxMbMap->yxSeqStart == pyxlink->next->pYxMbMap->yxSeqStart) {
                    if (val_YXstatus == Multi_Val_1) {
                        continue;
                    } else {
                        val_YXstatus = MultiVal2OneYX(pYxMbMap, pbuf);
                    }
                } else {
                    if (val_YXstatus == Multi_Val_0) {
                        val_YXstatus = MultiVal2OneYX(pYxMbMap, pbuf);
                    }
                    if (val_YXstatus == Multi_Val_0 || val_YXstatus == Multi_Val_1) { //存在多个数据取或得值位1
                        if (val_YXstatus == Multi_Val_0) {
                            *pbuf = 0;
                        } else {
                            *pbuf = 1;
                        }
                        switchBytes((uchar*)pbuf, 2);
                        mbRegSurplus--;//剩余个数减去一个寄存器
                        if (mbRegSurplus <= 0) {
                            break;
                        }
                        pbuf++;//buf偏移一个寄存器
                        val_YXstatus = NOT_Multi;
                        continue;
                    }
...
void startModbusSerail(void)
{
    uchar sbuf[255];
    for (;;) {
        for (uint i=CFG_SCREEN_NUM; i<Serail_count; i++) {//循环访问已打开的串口
            memset(sbuf, 0, sizeof(sbuf));
...
enum VAL_STATUS MultiVal2OneYX(TY_YX_MB_MAP *pYxMbMap, ushort *pval) //多个值对应一个遥信量
{
    ...
    long long val_u64 = 0;
    double val_tmp;
    if (pYxMbMap->dataType == 0) {
        if (*pval) {
            val_tmp = 1;
        } else {
            val_tmp = 0;
        }
    } else if (pYxMbMap->dataType == 1) { //short(16位)
//        switchBytes((uchar*)pval, sizeof(val_u16));
        memcpy(&val_u16, pval, sizeof(val_u16));
        val_tmp = (double)val_u16;
    } else if (pYxMbMap->dataType == 2) { //float(32位)
//        switchBytes((uchar*)pval, sizeof(val_f32));
        memcpy(&val_f32, pval, sizeof(val_f32));
        val_tmp = (double)val_f32;
    } else if (pYxMbMap->dataType == 3) { //long long(64位)
//        switchBytes((uchar*)pval, sizeof(val_u64));
        memcpy(&val_u64, pval, sizeof(val_u64));
        val_tmp = (double)val_u64;
    } else {
        printf("\nMultiVal2OneYX Seq:%d, Reg:%d, Addr:%d error type!\n", pYxMbMap->yxSeqStart, pYxMbMap->mbRegStart, pYxMbMap->mbDevAddr);
        return NOT_Multi;
    }
    if (val_tmp >= CFG_LIMIT_VALUE_YC2YX) {
        if (pYxMbMap->bit_start == 0xFF) { //不使用位
            return Multi_Val_1;
        } else { //使用位
            return ((*pval & (1 << pYxMbMap->bit_start)) > 0 ? Multi_Val_1 : Multi_Val_0);
        }
    } else {
        return Multi_Val_0;
    }
...
//将寄存器值转换成遥信量
void convertMbReg2Yx(uchar* yxs, int screen)
{
    ushort i, *pval;
    uchar buf[255];
    enum VAL_STATUS val_YXstatus = NOT_Multi;
    memset(buf, 0, sizeof(buf));
    pTY_YX_MB_MAP_LINK pyxlink = &yxMbMapLink_Simscreen[screen];
    TY_YX_MB_MAP* pYxMbMap;
    ...
    for (i=0; pyxlink != NULL; pyxlink=pyxlink->next) {
        ...
        if (pyxlink->next != NULL && pYxMbMap->yxSeqStart == pyxlink->next->pYxMbMap->yxSeqStart) { //功能：多个数据取或对应一个遥信
            if (val_YXstatus == Multi_Val_1) {
                continue;
            } else {
                val_YXstatus = MultiVal2OneYX(pYxMbMap, pval);
            }
        } else {
            ...
            if (val_YXstatus == Multi_Val_0 || val_YXstatus == Multi_Val_1) { //存在多个数据取或得值位1
                if (val_YXstatus == Multi_Val_0) {
                    setOneYx(yxs, pYxMbMap->yxSeqStart, convert2YxStatus(0));
                } else {
                    setOneYx(yxs, pYxMbMap->yxSeqStart, convert2YxStatus(1));
                }
                val_YXstatus = NOT_Multi;
                continue;
            }
...
//将寄存器值装换位遥测量
void convertMbReg2Yc(uchar* ycs, int screen)
{
    float val;
    pTY_YC_MB_REMAP_LINK pyclink = &ycMbREMapLink_Simscreen[screen];
    TY_YC_MB_REMAP* pYcMbREMap;

    for (; pyclink != NULL; pyclink = pyclink->next) {
        pYcMbREMap = pyclink->pYcMbREMap;
        val = (float)getCalcValue(pYcMbREMap->formula, ycMbMapLink_Simscreen[screen]);
        printf("mbaddr:%d val:%.3f\n", pYcMbREMap->ycModbusAddr, val);
...
DWORD WINAPI CdtBroadCastThread(LPVOID lpThreadParameter)
{
    ...
    if (n >= CFG_SCREEN_NUM || n < 0) {
        printf("CreateThreadId error.\n");
        exit(0);
    }
    printf("CDT ThreadId: %d create success\n", n + 1);
    screenAllOpen(Serail_NUM[n]); //Inited all open
    screenAllLight(Serail_NUM[n]);//Light up the screen
    for (;;) {
        if (yxFlag_Simscreen & (1 << n)) {
            convertMbReg2Yx(yxs, n);
            sendBatchYX(1, yxs, 120, Serail_NUM[n]);
            Sleep(serailParam.simFlushT);
        }
        if (ycFlag_Simscreen & (1 << n)) {
            convertMbReg2Yc(ycs, n);
            sendBatchYC(1, ycs, 120, Serail_NUM[n]);
            Sleep(serailParam.simFlushT);
        }
    }
}
void startCdtBroadCast(void)
{
    ...
    int i;
    int *IdParam;
    for (i = 1; i <= CFG_SCREEN_NUM; i++) {
        IdParam = (int *)malloc(sizeof(int));
        *IdParam = i;
        HANDLE h1 = CreateThread(NULL, 0, CdtBroadCastThread, (LPVOID)IdParam, 0, &lpThreadId);
